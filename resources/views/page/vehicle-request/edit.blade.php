  ...
  <div class="row">
    <div class="col-md-10">
      <div class="card">
        ...
        <div class="card-body">
          <form method="post" action="{{ route('update-vehicle-request', encrypt($data->id)) }}" enctype="multipart/form-data"> @csrf
            ...
            <div class="row">
              <div class="col-md-5">
                <div class="form-group row">
                  ...
                  <div class="col-lg-9">
                    <input type="date" name="request_date" class="form-control" value="{{ (old('request_date')) ? old('request_date') : date('Y-m-d', strtotime($data->request_date)) }}" required>
                  ...
                  <div class="col-lg-9">
                    <select name="vehicle_id" class="form-control select-search" required>
                      ...
                      @foreach($vehicle as $obj)
                        <option value="{{$obj->id}}" {{ (old('vehicle_id') == $obj->id) ? "selected" : "" }} {{ ($data->vehicle_id == $obj->id) ? "selected" : "" }}>{{$obj->name}}</option>
                      @endforeach
                    </select>
                  ...
                  <div class="col-lg-9">
                    <select name="driver_id" class="form-control select-search" required>
                      ...
                      @foreach($driver as $obj)
                        <option value="{{$obj->id}}" {{ (old('driver_id') == $obj->id) ? "selected" : "" }} {{ ($data->driver_id == $obj->id) ? "selected" : "" }}>{{$obj->name}}</option>
                      @endforeach
                    </select>
              ...
              <div class="col-md-7">
                <div class="form-group row">
                  ...
                  <div class="col-lg-9">
                    <textarea name="description" class="form-control" id="" cols="30" rows="6" maxlength="100">{{ (old('description')) ? old('description') : $data->description }}</textarea>
